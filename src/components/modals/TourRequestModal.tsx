...
export function TourRequestModal() {
  ...
  const buildMessage = () => {
    ...
    return [
      ...
      `${L.pax}: ${pax}`,
      form.budget !== null
        ? `${L.budget}: ${d.modal.fields.budgetOpts[form.budget]}`
        : null,
            ...
            {step === 0 ? (
              <div className="flex flex-col gap-6">
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <Field label={d.modal.fields.date} htmlFor={`${uid}-date`}>
                    <DateInput
                      id={`${uid}-date`}
                      value={form.date}
                ...
                <Field label={d.modal.fields.budget} htmlFor={`${uid}-budget`}>
                  <Select
                    id={`${uid}-budget`}
                    options={d.modal.fields.budgetOpts}
                    value={form.budget}
                    onChange={(i) => set("budget", i)}
              ...
              <div className="flex flex-col gap-6">
                <div className="grid gap-6 sm:grid-cols-2">
                  ...
                  <Field label={d.modal.fields.country} htmlFor={`${uid}-country`}>
                    <TextInput
                      id={`${uid}-country`}
                      value={form.country}
                ...
                <Field label={d.modal.fields.notes} htmlFor={`${uid}-notes`}>
                  <TextArea
                    id={`${uid}-notes`}
                    rows={4}
                    value={form.notes}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center justify-between rounded-lg border border-line px-1.5 py-1.5">
        <StepBtn onClick={() => onChange(Math.max(min, value - 1))} disabled={value <= min} minus />
        <span className="font-display text-[0.95rem] font-medium tabular-nums text-ink">
          {value}
        </span>
  ...
  return (
    ...
    >
      <svg viewBox="0 0 12 12" className="h-3 w-3" fill="none" aria-hidden="true">
        ...
        {!minus && <path d="M6 1.5v9" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />}
      </svg>
    </button>
  );
}
